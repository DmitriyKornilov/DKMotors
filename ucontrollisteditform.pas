...
begin
  ...
  CanFormClose:= SQLite.ControlUpdate(MotorID, STrim(NoteMemo.Text));
  ModalResult:= mrOK;
end;
...
begin
  ...
  SQLite.MotorListToControl(NameIDs[MotorNameComboBox.ItemIndex], MotorNumberLike,
...
begin
  ...
  SQLite.MotorListOnControl(MotorID, MotorIDs, MotorNums, Series);
  VSTTable.ValuesClear;
  VSTTable.SetColumn('Номер', MotorNums);
  VSTTable.SetColumn('Партия', Series);
  VSTTable.Draw;
  ...
  NoteMemo.Text:= SQLite.ControlNoteLoad(MotorID);
end;
