...
begin
  ...
  FWriter.AddCellBGColorIndex(R, C, MONTHNAME_COLOR_INDEX);
  FWriter.WriteText(R+1, C, R+3, C, 'Кален- дарных', cbtOuter);
  FWriter.AddCellBGColorIndex(R+1, C, MONTHNAME_COLOR_INDEX);
  C:= C+1;
  FWriter.WriteText(R+1, C, R+3, C, 'Рабочих', cbtOuter);
  FWriter.AddCellBGColorIndex(R+1, C, MONTHNAME_COLOR_INDEX);
  C:= C+1;
  FWriter.WriteText(R+1, C, R+3, C, 'Выход- ных' , cbtOuter);
  FWriter.AddCellBGColorIndex(R+1, C, MONTHNAME_COLOR_INDEX);
  C:= C+1;
  FWriter.WriteText(R+1, C, R+3, C, 'Празд- ничных', cbtOuter);
  FWriter.AddCellBGColorIndex(R+1, C, MONTHNAME_COLOR_INDEX);
  C:= C+1;
