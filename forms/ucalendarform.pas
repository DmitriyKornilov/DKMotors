...
begin
  ...
  SpecDays:= DataBase.LoadCalendarSpecDays(BD, ED);
...
begin
  ...
  Calendar:= DataBase.LoadCalendar(BD, ED);
  try
...
begin
  ...
  ED:= DataBase.LoadWorkDaysPeriodEndDate(BD, N);
...
begin
  ...
  DataBase.DeleteCalendarSpecDay(SpecDays.Dates[Ind]);
...
begin
  ...
  begin
    if IsCopyDates then
    begin
      DataBase.WriteCalendarSpecDays(SelectedDates, SelectedStatus);
      RefreshCalendar;
    end
    else begin
      SelectedDates:= nil;
